...
namespace HILibrary
{
    public class HIUnitTest
    {
        ...
        public static void Run(HIUnitTest[] tests, Func<HIUnitTest, int> testCode)
        {
            ...
            foreach (var test in tests)
            {
                ...
                catch
                {
                    ...
                    {
                        Console.WriteLine(testNotEqual, test.Text, test.targetValue, nameof(Exception));
                        failed++;
                    }
                }
            }
            Console.WriteLine(testEnd, testMethodName, tests.Length - failed, failed);
        }
        ...
        public int AssertEqual(object result)
        {
            ...
            {
                Console.WriteLine(testNotEqual, Text, nameof(Exception), result);
                return 1;
            }
            if (!targetValue.TEquals(result))
            {
                Console.WriteLine(testNotEqual, Text, targetValue, result);
                return 1;
            }
            return 0;
        }
        ...
        private readonly object targetValue;    // 유닛 테스트 실행 시 예상되는 결과값
    }
}